...
class Box_plot:
    ...
    def _add_individual_dots(self, displayed_data, x_loc, cmaps, uncertainty_types_list,):  
        ...
        self.ax.plot(  
            ...
            alpha=self.config.dot_alpha_normal  
        )  
    ...
    def _format_plot(
            ...
            for idx_text, perc_info in enumerate(self.all_sample_percs):  
                if show_sample_info == "Average":  
                    self.ax.text(  
                        ...
                        self.max_bin_height * 0.8,  
                        r"$\bf{PSB}$" + ": \n" + r"${} \pm$".format(perc_info[0]) + "\n" + r"${}$".format(perc_info[1]),  
                        verticalalignment="bottom",  
                        horizontalalignment="center",  
                        fontsize=self.config.sample_info_fontsize,  # Replace hardcoded 25  
                    )  
        ...
        # Replace hardcoded subplot adjustments  
        plt.subplots_adjust(bottom=self.config.subplot_bottom)  # Replace 0.15  
        plt.subplots_adjust(left=self.config.subplot_left)      # Replace 0.15  
        ...
        self.ax.legend(  
            ...
            fontsize=self.config.legend_fontsize,        # Replace hardcoded 20  
            ncol=num_cols_legend,  
            columnspacing=self.config.legend_columnspacing,  # Replace hardcoded 2  
            loc="upper center",  
            bbox_to_anchor=self.config.legend_bbox_anchor,   # Replace hardcoded (0.5, 1.18)  
            fancybox=True,  
            shadow=False,  
        )  
    
        # Replace hardcoded figure settings  
        if save_path is not None:  
            plt.gcf().set_size_inches(*self.config.figure_size)  # Replace hardcoded (16.0, 10.0)  
            plt.tight_layout()  
            plt.savefig(save_path, dpi=self.config.dpi,          # Replace hardcoded 600  
                    bbox_inches=self.config.bbox_inches,      # Replace hardcoded "tight"  
                    pad_inches=self.config.pad_inches)        # Replace hardcoded 0.1  
            plt.close()  
        else:  
            plt.gcf().set_size_inches(*self.config.figure_size)  # Replace hardcoded (16.0, 10.0)  
            plt.show()  
    ...
    def box_plot_comparing_q(
        ...
        for idx, q_value in enumerate(x_axis_labels):
            ...
            for j in range(len(model_data)):
                ...
                rect = self.ax.boxplot(displayed_data, positions=x_loc, sym="", 
                                    widths=width, showmeans=True, patch_artist=True)
                ...
                if show_sample_info != "None":
                    ...
                    if show_sample_info == "All":
                        (x_l, y), (x_r, _) = rect["caps"][-1].get_xydata()
                        x_line_center = (x_l + x_r) / 2
                        self.all_sample_label_x_locs.append([x_line_center, y + 0.5])
                        self.all_sample_percs.append(percent_size)
