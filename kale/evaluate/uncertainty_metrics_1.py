...
def _base_uncertainty_evaluation(  
    bin_predictions: Dict[str, pd.DataFrame],  
    uncertainty_pairs: List,  
    num_bins: int,  
    targets: List[int],  
    num_folds: int,  
    metric_calculator_func: callable,  
    combine_middle_bins: bool = False,  
    **kwargs  
) -> Dict:  
    """  
    Base framework for uncertainty evaluation metrics.  
      
    Args:  
        metric_calculator_func: Function that processes fold data (bin_wise_bound_eval,   
                               bin_wise_errors, or bin_wise_jaccard)  
    """  
    if combine_middle_bins:  
        num_bins = 3  
      
    # Initialize common data structures  
    containers = _initialize_evaluation_containers(num_bins, targets)  
    ...
    for i, (model, data_structs) in enumerate(bin_predictions.items()):  
        for uncert_pair in uncertainty_pairs:  
            ...
            # Initialize fold-wise containers  
            fold_containers = _initialize_fold_containers(num_bins, targets)  
              
            for fold in range(num_folds):  
                # Extract fold data  
                fold_data = _extract_fold_data(data_structs, fold, uncertainty_type)  
                  
                # Call specific metric calculator  
                result_dict = metric_calculator_func(fold_data, model, fold, **kwargs)  
                  
                # Aggregate results  
                _aggregate_fold_results(result_dict, fold_containers, targets, num_bins)  
              
            # Store final results  
            _store_final_results(containers, model, uncertainty_type, fold_containers)  
      
    return containers

def _initialize_evaluation_containers(num_bins: int, targets: List[int]) -> Dict:  
    """Initialize common evaluation result containers."""  
    return {  
        'main_results': {},  
        'target_separated': {},  
        'target_sep_foldwise': [{} for _ in range(len(targets))],  
        'target_sep_all': [{} for _ in range(len(targets))],  
        'target_nosep': {}  
    }  
  
def _initialize_fold_containers(num_bins: int, targets: List[int]) -> Dict:  
    """Initialize fold-wise result containers."""  
    return {  
        'fold_mean_targets': [],  
        'fold_mean_bins': [[] for _ in range(num_bins)],  
        'fold_all_bins': [[] for _ in range(num_bins)],  
        'fold_concat_targets_sep_foldwise': [[[] for _ in range(num_bins)] for _ in range(len(targets))],  
        'fold_concat_targets_sep_all': [[[] for _ in range(num_bins)] for _ in range(len(targets))],  
        'fold_concat_targets_nosep': [[] for _ in range(num_bins)]  
    }

def _extract_fold_data(data_structs: pd.DataFrame, fold: int, uncertainty_type: str) -> Dict:  
    """Extract fold-specific data for processing."""  
...
def _aggregate_fold_results(result_dict: Dict, fold_containers: Dict, targets: List[int], num_bins: int):  
    """Aggregate results from metric calculator into fold containers."""  
    fold_containers['fold_mean_targets'].append(result_dict["mean all targets"])  
      
    for idx_bin in range(len(result_dict["mean all bins"])):  
        fold_containers['fold_mean_bins'][idx_bin].append(result_dict["mean all bins"][idx_bin])  
          
        # Handle target-separated aggregation  
        for target_idx in range(len(targets)):  
            fold_containers['fold_concat_targets_sep_foldwise'][target_idx][idx_bin].extend(  
                result_dict["all bins concatenated targets seperated"][target_idx][idx_bin]  
            )  
  
def _store_final_results(containers: Dict, model: str, uncertainty_type: str, fold_containers: Dict):  
    """Store final aggregated results in main containers."""  
    key = f"{model} {uncertainty_type}"  
      
    # Reverse ordering for worst-to-best display  
    containers['main_results'][key] = fold_containers['fold_mean_bins'][::-1]  
    containers['target_separated'][key] = fold_containers['fold_all_bins'][::-1]

def evaluate_bounds(estimated_bounds, bin_predictions, uncertainty_pairs, num_bins, targets, num_folds=8, **kwargs):  
    """Refactored bounds evaluation using base framework."""  
    
    def bounds_calculator(fold_data, model, fold, **calc_kwargs):
        error_bounds = calc_kwargs['estimated_bounds'][model + " Error Bounds"]  
        fold_bounds = strip_for_bound(  
            error_bounds[error_bounds["fold"] == fold][fold_data['uncertainty_type'] + " Uncertainty bounds"].values  
        )  

        return bin_wise_bound_eval(  
            fold_bounds,  
            fold_data['fold_errors'],  
            fold_data['fold_bins'],  
            calc_kwargs['targets'],  
            fold_data['uncertainty_type'],  
            **calc_kwargs  
        )  
      
    return _base_uncertainty_evaluation(  
        bin_predictions, uncertainty_pairs, num_bins, targets, num_folds,  
        bounds_calculator, estimated_bounds=estimated_bounds, **kwargs  
    )  
  
def get_mean_errors(bin_predictions, uncertainty_pairs, num_bins, targets, num_folds=8, **kwargs):  
    """Refactored error evaluation using base framework."""  
      
    def errors_calculator(fold_data, model, fold, **calc_kwargs):  
        return bin_wise_errors(  
            fold_data['fold_errors'],  
            fold_data['fold_bins'],  
            calc_kwargs['num_bins'],  
            calc_kwargs['targets'],  
            fold_data['uncertainty_type'],  
            **calc_kwargs  
        )  
    ...
    return _base_uncertainty_evaluation(  
        bin_predictions, uncertainty_pairs, num_bins, targets, num_folds,  
        errors_calculator, **kwargs  
    )  
  
def evaluate_jaccard(bin_predictions, uncertainty_pairs, num_bins, targets, num_folds=8, **kwargs):  
    """Refactored Jaccard evaluation using base framework."""  
      
    def jaccard_calculator(fold_data, model, fold,  **calc_kwargs):  
        return bin_wise_jaccard(  
            fold_data['fold_errors'],  
            fold_data['fold_bins'],  
            calc_kwargs['num_bins'],  
            calc_kwargs.get('num_bins_for_quantiles', calc_kwargs['num_bins']),  
            calc_kwargs['targets'],  
            fold_data['uncertainty_type'],  
            **calc_kwargs  
        )  
...
def get_mean_errors(bin_predictions, uncertainty_pairs, num_bins, targets, num_folds=8, **kwargs):  
    ...
    def errors_calculator(fold_data, **calc_kwargs):  
        return bin_wise_errors(  
            fold_data['fold_errors'],  
            fold_data['fold_bins'],  
            calc_kwargs['num_bins'],  
...
def evaluate_jaccard(bin_predictions, uncertainty_pairs, num_bins, targets, num_folds=8, **kwargs):  
    ...
    def jaccard_calculator(fold_data, **calc_kwargs):  
        return bin_wise_jaccard(  
            fold_data['fold_errors'],  
            fold_data['fold_bins'],  
            calc_kwargs['num_bins'],  
